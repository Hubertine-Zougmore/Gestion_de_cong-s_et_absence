...
@php
    ...
    $mesDemandesSecretaire = $mesDemandesSecretaire ?? collect(); // Correction du nom
...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Tableau de Bord du Secrétaire Général</h1>

    <!-- En-tête -->
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <h1 class="text-2xl font-bold text-gray-900 mb-2">Bienvenue, {{ $user->prenom ?? 'Utilisateur' }} {{ $user->nom ?? '' }}</h1>
        <p class="text-gray-600">Vous êtes connecté en tant que <strong>Secrétaire Général</strong>.</p>
    </div>
    ...
    <div class="bg-white shadow rounded-lg mb-6">
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px">
                ...
                <button class="tab-button px-6 py-3 text-sm font-medium border-b-2 border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap" data-tab="mes-demandes">
                    Mes Demandes
                    @if($mesDemandesSecretaire->count() > 0)
                        <span class="ml-2 bg-blue-100 text-blue-800 text-xs font-medium px-2.5 py-0.5 rounded-full">{{ $mesDemandesSecretaire->count() }}</span>
                    @endif
                </button>
        ...
        <div id="demandes-traiter" class="tab-content p-6">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-xl font-semibold">Demandes à Traiter (congés annuel)</h2>
                <div class="bg-purple-50 border border-purple-200 rounded-lg px-3 py-1">
                    <p class="text-sm text-purple-700">
                        📋 Je peux traiter les demandes de congés annuel
                    </p>
                </div>
            </div>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            @if($demande->delai_restant_jours > 24)
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                                    {{ ceil($demande->delai_restant_jours / 24) }} j
                                                </span>
                                            @elseif($demande->delai_restant_jours > 0)
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                                    {{ $demande->delai_restant_jours }} h
                                                </span>
                                            @else
                                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                                                    {{ abs($demande->delai_restant_jours) }} h de retard
                                                </span>
                                            @endif
                                        </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    ...
                                    @if($demande->peut_traiter)
                                        <a href="{{ route('secretaire_general.demande-show', $demande) }}" 
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">Aucune demande à traiter</h3>
                    <p class="mt-1 text-sm text-gray-500">Toutes les demandes de congés annuel ont été traitées.</p>
                </div>
            @endif
        </div>
            ...
            @if($mesDemandesSecretaire->count() > 0)
                <div class="overflow-x-auto">
                    <table class="min-w-full">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($mesDemandesSecretaire as $demande)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
