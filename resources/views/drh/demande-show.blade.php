...
<div class="max-w-5xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    <!-- Bouton de retour -->
   <div class="mb-6">
        <a href="{{ route('drh.dashboard') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-700 border border-transparent rounded-lg font-semibold text-sm text-white uppercase tracking-widest hover:from-blue-600 hover:to-blue-800 transition-all duration-200 shadow-md hover:shadow-lg">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-100 shadow-sm">
                    <div class="flex items-center mb-5">
                        ...
                        <h3 class="ml-4 text-lg font-semibold text-blue-900">Informations employé</h3>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex justify-between items-center py-2 border-b border-blue-100">
                            ...
                            <a href="mailto:{{ $demande->user->email ?? '' }}" class="text-blue-600 hover:text-blue-800 transition-colors font-medium">
                                {{ $demande->user->email ?? '' }}
                            </a>
                        </div>
                       ...
                       <div class="flex justify-between items-center py-2">
                            ...
                            <span class="text-blue-900">
                                @if($demande->user && $demande->user->role)
                                    @if(is_object($demande->user->role))
                                        {{ $demande->user->role->nom ?? 'Non spécifié' }}
                                    @else
                                        {{ $demande->user->role }}
                                    @endif
                                @else
                    ...
                    <div class="space-y-4">
                        <div class="flex justify-between items-center py-2 border-b border-gray-100">
                            ...
                            <span class="inline-flex px-3 py-1 text-xs font-semibold rounded-full 
                                {{ ($demande->type === 'conge') ? 'bg-green-100 text-green-800' : 
                                   (($demande->type === 'absence') ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800') }}">
                        ...
                        <div class="flex justify-between items-center py-2">
                            ...
                            <span class="text-sm text-gray-900">
                                <span class="font-semibold {{ $demande->est_exactement_72h ? 'text-blue-600' : 
                                      ($demande->est_moins_72h ? 'text-green-600' : 'text-red-600') }}">
                                    {{ $demande->duree_heures }}h
                                </span>
    ...
    <!-- INDICATION DES PERMISSIONS -->
    @if($demande->statut === 'en_attente')
    <div class="mt-6 p-5 rounded-xl border 
        @if($demande->peut_traiter) bg-green-50 border-green-200 
        @elseif($demande->president_doit_traiter) bg-purple-50 border-purple-200 
        ...
        <div class="flex items-start">
            <div class="flex-shrink-0">
                ...
                @elseif($demande->president_doit_traiter)
            ...
            <div class="ml-3">
                <h3 class="text-sm font-medium">
                    @if($demande->peut_traiter)
                        <span class="text-green-800">Vous pouvez traiter cette demande</span>
                    @elseif($demande->president_doit_traiter)
                        <span class="text-purple-800">Cette demande doit être traitée par le président</span>
                    @else
                        <span class="text-yellow-800">Vous ne pouvez pas traiter cette demande</span>
                    @endif
                </h3>
                <div class="mt-2 text-sm">
                    @if($demande->peut_traiter)
                        <p class="text-green-700">
                            @if($demande->est_exactement_72h)
                                Raison: Durée = 72h (autorisée pour toutes les directions)
                            @else
                                Raison: Durée < 72h et demande de votre direction
                            @endif
                        </p>
                    @elseif($demande->president_doit_traiter)
                        <p class="text-purple-700">Raison: Durée > 72h (délégation présidentielle)</p>
                    @else
                        <p class="text-yellow-700">Raison: Durée < 72h mais demande ne faisant pas partie de votre direction</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="mt-8 bg-gradient-to-br from-gray-50 to-blue-50 p-6 rounded-xl border border-gray-200">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="border border-green-200 rounded-xl p-5 bg-white shadow-sm">
                ...
                <form action="{{ route('drh.demande.approuver', $demande) }}" method="POST">
                    @csrf
                    <div class="mb-4">
                        ...
                        <textarea name="commentaire" id="commentaire_approbation" rows="3" 
                                class="mt-1 block w-full border border-green-300 rounded-lg shadow-sm p-3 focus:ring-green-500 focus:border-green-500 transition duration-150"></textarea>
                    </div>
                    <button type="submit" 
            ...
            <div class="border border-red-200 rounded-xl p-5 bg-white shadow-sm">
                ...
                <form action="{{ route('drh.demande.rejeter', $demande) }}" method="POST">
                    @csrf
                    <div class="mb-4">
        ...
        <div class="bg-white p-5 rounded-lg border border-blue-100">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-sm text-gray-900">{{ $demande->traitee_par ? $demande->traitePar->prenom . ' ' . $demande->traitePar->nom : 'Système' }}</p>
                </div>
                <div>
            ...
            @if($demande->commentaire_drh)
                <div class="mt-4 pt-4 border-t border-gray-100">
                    ...
                    <p class="text-sm text-gray-900 mt-1">{{ $demande->commentaire_drh }}</p>
                </div>
            @endif
        </div>
    </div>
